const fs = require('fs'), path = require('path'), fsp = require('fs/promises'),
    ...
    NotesResult = require("./NotesResult"),
    Timer = require("./Timer");
const Parameters = require('./Parameters');

module.exports = class NotesSection extends Section {
    ...
    constructor(root_path, tagsFilePath) {
        ...
        this._rootElement.classList.add("notes");
        this._rootElement.innerHTML = this._loadSectionHtmlContent(__dirname + "/NotesSection.html");
        ...
        tinymce.init({
            selector: '#note-container .description',
            menubar: false,
            inline: true,
            toolbar_persist: true,
            // skin: "oxide-dark",
            // link_context_toolbar: true,
            fixed_toolbar_container: "#note-container .tinymce-toolbar-container",
            ...
            setup: editor => {
                editor.on('Paste Change input Undo Redo', () => {
                    ...
                    editorChangeHandlerId = setTimeout(() => {
                        this.#currentlyDisplayedNotes.description = editor.getContent();
                        this.#currentlyDisplayedNotes.obj.storeLvDatas();
                    }, 800);
                });

                // Handling of local file drop on note (like video file)
    ...
    _registerEventHandlers() {
                    ...
                    case "newNote":
                        ...
                        fs.mkdir(path.join(ROOT_FOLDER, folderPath), (err) => {
                            ...
                            // Creation du dossier réussie
                                ...
                                var jsonDatas = { "label": "Nouvelle note", "description": "", "tags": [] };
                                ...
                                fs.mkdir(path.join(ROOT_FOLDER, folderPath, "resources"), error => {
                                    ...
                                    // Creation du dossier réussie -> Update notes list
            ...
            if (result = event.target.closest(".result")) {
                ...
                // Click on button
                    ...
                    switch (target.dataset.action) {
                        ...
                        case "delete":
                            if (confirm("Supprimer la note ?\r" + result.label)) fs.rmdir(path.join(ROOT_FOLDER, result.path), { recursive: true }, (error) => {
                ...
                // Click on anything else => Open note -------------------------------------------
                else this.#openNote(result);
            }
        });

        // Click on note container
        this._rootElement.querySelector("#note-container").addEventListener("click", event => {
            var target;
            ...
            // Click qur un bouton (close, etc...)
                ...
                switch (target.dataset.action) {
                    // Close currently displayed note
                    case "close":
                        this.#currentlyDisplayedNotes = null;
                        this._rootElement.classList.remove("displayNote");
        ...
        // Double click on note container
        this._rootElement.querySelector("#note-container").addEventListener("dblclick", event => {
            var target;

            // Click sur un tab
            if (target = event.target.closest(".tab")) {
                switch (target.dataset.target) {
                    case "view": nw.Shell.openItem(path.join(ROOT_FOLDER, this.#currentlyDisplayedNotes.path)); break;

                    case "resources": nw.Shell.openItem(path.join(ROOT_FOLDER, this.#currentlyDisplayedNotes.path, 'resources')); break;
        ...
        // Type on note contenteditable (except description)
        var delayedTriggerIdx;
            ...
            elem.addEventListener("input", event2 => {
                ...
                delayedTriggerIdx = setTimeout(() => {
                    switch (event2.target.dataset.source) {
                        ...
                        case "label":
                            this.#currentlyDisplayedNotes.label = this.#currentlyDisplayedNotes.querySelector(".label").innerHTML = event2.target.innerHTML;
                            this.#currentlyDisplayedNotes.obj.storeLvDatas();
                            break;
            ...
            if (target = event.target.closest(".result")) {
                ...
                items.push(
                    ...
                    },
                    {
                        ...
                        icon: "trashcan.svg",
                        action: item => {
                            if (confirm("Supprimer cette note ?\r" + target.label)) {
                                fs.rmdir(path.join(ROOT_FOLDER, target.path), { recursive: true }, error => {
                                    if (error) {
                                        console.error(error);
                                        alert("Erreur lors de la supression de la note\r" + target.label);
                                    }
                                    else {
        ...
        this._rootElement.addEventListener("drop", event => {
            ...
            else if (this.#currentlyDisplayedNotes && this._rootElement.querySelector(".tabContent.resources.displayed") && event.dataTransfer.files.length > 0) {
        ...
        document.addEventListener("paste", event => {
            ...
            if (this.#currentlyDisplayedNotes) {
                    ...
                    // Un URL est collée
                        ...
                        item.getAsString(string => {
                            if (string.match(/^https?:/i)) {
                                fs.writeFile(path.join(ROOT_FOLDER, this.#currentlyDisplayedNotes.path, "resources", "Nouveau Site.url"), "[{000214A0-0000-0000-C000-000000000046}]\nProp3=19,11\n[InternetShortcut]\nIDList=\nURL=" + string + "\n", (err) => {
                                    if (err) console.log(err);
                                    else this.#updateNoteResourceFiles();
                                });
                            }
                        });
                    }
    ...
    #updateNoteResourceFiles() {
        document.body.classList.add("loading");

        var filesContainer = this._rootElement.querySelector(".tabContent .filesContainer");
        filesContainer.innerHTML = "";

        fs.readdir(path.join(ROOT_FOLDER, this.#currentlyDisplayedNotes.path, "resources"), { withFileTypes: true }, (error, files) => {
            if (error) {
                console.log(error);
                ...
                files.forEach(dirent => {
                    if (dirent.isFile()) {
                        ...
                        fileElem.path = path.join(this.#currentlyDisplayedNotes.path, "resources", dirent.name);
    ...
    #openNote(result) {
        this.#currentlyDisplayedNotes = result;

        this._rootElement.querySelector(".noteTopbar .left").innerHTML = result.label;
        this._rootElement.querySelector(".description").innerHTML = result.description;

        this.#updateNoteResourceFiles();

        this._rootElement.classList.add("displayNote");
    ...
    #writeFile(fileBlob, fileName, alterNameToSave) {
        ...
        reader.addEventListener("load", event => {
            var filePath = path.join(this.#currentlyDisplayedNotes.path, "resources", fileName);
                ...
                if (fs.existsSync(path.join(ROOT_FOLDER, filePath))) {
                    if (alterNameToSave) {
                        ...
                        for (var i = 2; i < 1000; i++) {
                            filePath = path.join(this.#currentlyDisplayedNotes.path, "resources", fileName + i + ext);
        ...
        fs.readdir(pathToExplore, { withFileTypes: true }, (err, filesList) => {
            ...
            filesList.forEach(file => {
                ...
                //If not, start analizing if it's a directory
                    ...
                    lvDatas = NotesResult.getLvDatas(dirPath);
    ...
    displayResults(pathToExplore, goingBack, stopTagSearch) {
        ...
        this._rootElement.querySelector(".topbar .path").innerHTML = "Notes<span class='separateur'>></span>" + path.relative(this._root_path, pathToExplore).split("\\").join("<span class='separateur'>></span>");
        ...
        fsp.readdir(path.join(ROOT_FOLDER, pathToExplore), { withFileTypes: true })
            .then(files => {
                ...
                if (pathToExplore !== this._root_path) {
                    resultElem = new NotesResult("..", true, path.dirname(pathToExplore), this).toElement();
                ...
                files.forEach(file => {
                    ...
                    resultElem = new NotesResult(file.name, file.isDirectory(), pathToExplore + "\\" + file.name, this).toElement();
    ...
    _displayTagSearchResults() {
        ...
        if (filteredDirPathList !== null) filteredDirPathList.forEach(dirPath => this._rootElement.querySelector(".resultsContainer").appendChild(new NotesResult(path.basename(dirPath), true, dirPath, this).toElement()));
    }
    ...
    display() {
        ...
        SectionSelector.querySelector(".tab.notes").classList.add("selected");
